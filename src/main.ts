...
const state: AppState = { fileName: '', demo: false, destination: 'apple', repairs: { ...initialRepairs } };
...
function shell(content: string): string {
  return `${state.demo ? `<aside class="demo-bar" aria-label="Demo mode"><span><strong>Demo</strong> — sample data, nothing is saved</span><span class="demo-actions"><button type="button" data-action="reset-demo">Reset demo</button><button type="button" data-action="start-real">Start for real</button></span></aside>` : ''}
  ...
  ${content}
  <footer><div><strong>ICS Intake Checker</strong><p>Check and repair an ICS file before calendar import.</p></div><nav aria-label="Footer navigation">${navLink('/privacy', 'Privacy')}${navLink('/terms', 'Terms')}<a href="https://hello-factory.sociobot.in" rel="external">Built by Param Factory</a></nav><p class="build-id">v1.0 · build 2026.08</p><p class="art-credit">Original generated illustration.</p></footer>`;
}
...
function toolMarkup(): string {
  if (!state.inspection) {
    return `<section class="intake" id="checker" aria-labelledby="checker-title">
      <div class="section-kicker">Private intake deck</div><h2 id="checker-title">Open a calendar file here</h2>
  ...
  return `<section class="workspace" id="checker" aria-labelledby="checker-title">
    <div class="file-ribbon"><div><span class="section-kicker">Inspection complete</span><h2 id="checker-title">${escapeHtml(state.fileName)}</h2></div><button class="quiet-button" type="button" data-action="clear">Forget this file</button></div>
...
function landing(): string {
  ...
  return shell(`<main id="main">
    <section class="hero" aria-labelledby="page-title"><div class="hero-copy"><span class="eyebrow">Calendar attachment preflight</span><h1 id="page-title" tabindex="-1">Check an ICS file before calendar import</h1><p class="lede">For people who receive calendar files and need to spot risky details before choosing the right calendar.</p><div class="hero-action"><a class="primary-button" href="/demo" data-route>Try it with sample data</a><span>See timezones, repeats, people, alarms, and duplicate risks.</span></div><ul class="plain-facts"><li>Event details stay in this browser.</li><li>Works offline after the first visit.</li><li>Free. No account.</li></ul></div>
      <figure class="hero-art"><picture><source srcset="/assets/inspection-landscape.webp" type="image/webp"/><img src="/assets/inspection-landscape.jpg" width="768" height="512" alt="A glass calendar file passes over a lit inspection table before import." fetchpriority="high" decoding="async"/></picture><figcaption>Inspect the attachment. Route only the copy you trust.</figcaption></figure>
    </section>
    ${toolMarkup()}
    <section class="how" aria-labelledby="how-title"><div><span class="section-kicker">Three stops</span><h2 id="how-title">How the preflight works</h2></div><ol><li><b>1</b><h3>Open the file</h3><p>Drop an attachment or choose it from this device.</p></li><li><b>2</b><h3>Read each finding</h3><p>Check times, repeats, people, alarms, links, and duplicates.</p></li><li><b>3</b><h3>Export a copy</h3><p>Apply optional cleanup and download for your calendar.</p></li></ol></section>
    <section class="boundaries" aria-labelledby="boundaries-title"><div><span class="section-kicker">Clear boundary</span><h2 id="boundaries-title">This checker does not access calendars</h2></div><div><p>It does not import events, sync accounts, or open links from a file.</p><p>It checks one attachment and prepares a new copy for you.</p></div></section>
  </main>`);
}

function demoPage(): string {
  pageTitle('Demo — ICS Intake Checker', 'Try ICS Intake Checker with a sample clinic and vendor calendar file.');
  return shell(`<main id="main"><section class="demo-heading"><span class="eyebrow">Sample inspection</span><h1 id="page-title" tabindex="-1">Inspect a sample calendar attachment</h1><p>The sample contains invitation details, attendee addresses, an alarm, a repeat rule, and a floating time.</p></section>${toolMarkup()}</main>`);
}
...
function infoPage(kind: 'privacy' | 'terms'): string {
  ...
  const body = privacy ? `<p>ICS Intake Checker processes calendar files inside your browser. It does not upload event details or contact links found in a file.</p><h2>Data stored on this device</h2><p>The app saves your latest opened file in browser storage so a refresh does not erase your work. Choose “Forget this file” to remove it. Demo data uses a separate in-memory workspace and is never saved.</p><h2>Network requests</h2><p>The installed app may request its own files and check for an updated app shell. It uses no analytics, advertising, third-party scripts, or remote fonts.</p><h2>Your choices</h2><p>You can clear this site's storage in browser settings. Downloaded calendar copies remain wherever you save them.</p>` : `<p>This free utility explains common ICS risks and creates a revised copy. You decide whether to import any file.</p><h2>No calendar access</h2><p>The app does not access, change, or sync your calendars. It cannot predict every calendar app's behavior.</p><h2>Your responsibility</h2><p>Review event times, recurrence, attendees, alarms, and destination before import. Keep the original attachment until you confirm the result.</p><h2>Warranty</h2><p>The software is provided under the MIT License without warranty. Do not use it as the only record for medical, legal, or emergency schedules.</p>`;
...
function bind(): void {
  document.querySelectorAll<HTMLAnchorElement>('[data-route]').forEach((link) => link.addEventListener('click', (event) => { event.preventDefault(); history.pushState({}, '', link.pathname); if (link.pathname === '/demo') parseSource(SAMPLE_ICS, 'sample-clinic-and-vendor.ics', false); else render(true); }));
  ...
  document.querySelector('[data-action="clear"]')?.addEventListener('click', () => { state.inspection = undefined; state.fileName = ''; void db('clear'); render(); live.textContent = 'The saved file was forgotten.'; });
  document.querySelector('[data-action="export"]')?.addEventListener('click', download);
  document.querySelector('[data-action="reset-demo"]')?.addEventListener('click', () => parseSource(SAMPLE_ICS, 'sample-clinic-and-vendor.ics', false));
  document.querySelector('[data-action="start-real"]')?.addEventListener('click', () => { state.inspection = undefined; state.fileName = ''; history.pushState({}, '', '/'); render(true); });
...
window.addEventListener('popstate', () => {
  if (window.location.pathname === '/demo' && !state.inspection) parseSource(SAMPLE_ICS, 'sample-clinic-and-vendor.ics', false);
  else render(true);
});
